...
class Scraping:
    ...
    @classmethod
    async def independent(cls):
        """Runs for gathering Independent news."""
        await asyncio.gather(cls.parse_main_data(c.INDEPENDENT, c.IND_TITLE, cls.ind_titles,
                                                 cls.ind_links))
        ...
        await asyncio.gather(Formatting.write("independent.txt", cls.ind_titles, cls.ind_subs,
                                                cls.ind_links, c.INDEPENDENT))
    
    @classmethod
    async def bbc(cls):
        """Gathers BBC News."""
        await asyncio.gather(cls.parse_main_data(c.BBC, c.BBC_TITLE, 
                                                 cls.bbc_titles, cls.bbc_links))
                                                 
        await asyncio.gather(cls.parse_sub_titles(cls.bbc_links, 
                                                  cls.bbc_subs, c.BBC, c.BBC_SUBS))
                                                  
        await asyncio.gather(Formatting.write("bbc.txt", cls.bbc_titles, cls.bbc_subs, 
                                              cls.bbc_links, c.BBC))
